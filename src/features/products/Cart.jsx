...
const Cart = () => {
  ...
  const total = (cart.reduce((sum, item) => item.quantity?sum+item.price*item.quantity:sum + item.price, 0)).toFixed(2);
  return (
    <div>
      ...
      <div className="flex justify-between mx-10">
        <button onClick={clearData} className="text-xl p-2 mx-40 font-bold bg-[#DC2626]  rounded hover:scale-105">
          Clear Data
        </button>
        ...
        >
          ...
          <div className=" flex flex-col items-center justify-center mx-10 gap-3">
            ...
            <div className=" flex  gap-2  ">
              <input value={item.quantity?item.quantity:1} onChange={(e)=>manageQuantity(item.id,e.target.value)}
